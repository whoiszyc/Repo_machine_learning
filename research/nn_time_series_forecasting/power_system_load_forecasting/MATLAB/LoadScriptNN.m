...
%%  ZYC comments and notes

% In fitting problems, you want a neural network to map between a data set of numeric inputs and a set of numeric targets.
% For example, estimating house prices from such input variables as tax rate, pupil/teacher ratio in local schools and crime rate;
% predicting a patient's bodyfat level based on body measurements
% In load forecasting, inputs are weather, date and historic load data and so on (input); outputs are forecasting data (target)

% The function genPredictors is to prepare the data set of input. The content is:
% 'DryBulb'    'DewPoint'                                                   (Weather information)
% 'Hour'    'Weekday'    'IsWorkingDay'                                     (Time information) 
% 'PrevWeekSameHourLoad'    'prevDaySameHourLoad'    'prev24HrAveLoad'      (Historic load data)

% For cyber attack, the best potential target is the historic load data since it is hard to do crosscheck
% Information of weather and time is public and easy to check
...
[num, text] = xlsread('Original/Holidays1.xls'); 
...
trainX = X(trainInd,:);  % (ZYC) Input from Predictors
trainY = data.SYSLoad(trainInd); % (ZYC) Target
...
% save test data
save('Result/testSet.mat','testDates','testX','testY')
% save train data
save('Result/trainSet.mat','trainDates','trainX','trainY')
...
% reTrain = false;
% if reTrain || ~exist('Result/NNModel.mat', 'file')
% %     net = newfit(trainX', trainY', 10);
%     net = feedforwardnet(5);
%     net.performFcn = 'mae';
%     net = train(net, trainX', trainY');
%     save('Result/NNModel.mat','net')
% else
%     load('Result/NNModel.mat')
% end
...
%% Convert the net to ONNX format (it seems this function is in 2019)
% filename = 'load_forecast.onnx';
% exportONNXNetwork(net,filename)

%% Forecast using Neural Network Model
% Once the model is built, perform a forecast on the independent test set. 

load('Result/testSet.mat')
